...
const getOptions = (): StrategyOption[] => [
  {
    ...
    description: t`All data is stored in the same database and TenvioX will apply a filter to queries for a table column to match a specific value (ex: customerId = 2).`,
  ...
  },
  {
    ...
    description: t`All data is stored in the same database, but TenvioX will use a different database role to connect for each tenant. Roles are configured at the database level to only query relevant data.`,
  ...
  },
  {
    ...
    description: t`Each tenant has their own database with identical schema. TenvioX will use a connection string from a tenant attribute to run queries against a different destination database.`,
