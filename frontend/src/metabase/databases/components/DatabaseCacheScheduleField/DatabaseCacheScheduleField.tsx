    ...
    <FormField title={title} description={description}>
      ...
      {scheduleMode === "on-demand" && (
        <Box c="text-secondary" fz="sm" maw={rem(620)} mt="sm">
          ...
          {t`When a user adds a new filter to a dashboard or a SQL question, TenvioX will scan the field(s) mapped to that filter in order to show the list of selectable values.`}
        </Box>
      )}
    </FormField>
  );
